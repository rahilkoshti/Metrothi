...
- **Sheet header (sticky):** Line badge, an accent eyebrow ("Nearest station" / "Default station" / "Station"), the station name, and a **Favorite** toggle that persists to `localStorage['metrothi-saved-stations']`. Below it a chip row: **line · walk distance · walk time**, visible at every snap; expanding appends an Interchange chip and a service-status chip when the line isn't running. The collapsed peek (118px) is sized to exactly this header.
- **Sheet body — actions:** A primary pair.
  - **Get Directions** (shown only at the user's *nearest* station) hands off to **Google Maps walking directions** — street-level navigation is not something this app models. Origin is the live fix where available; Google Maps falls back to the device's own location otherwise.
  - **Trip to here** (shown at any other station) seeds the GO planner with that station as the destination, via the in-app `home-plan-trip` event so the map stays mounted beneath.
  - **Station Details** expands the sheet to its **full** snap rather than routing away — the detail already lives further down the same sheet.
- **Upcoming Trains preview:** Directly under the actions, the next departure in each direction, for every line the station serves (2 cards at a through-station, 4 at an interchange, 1 per line at a terminal). Each card carries the destination terminal, the clock time, and minutes-to-go; a green "Live" marker appears while any of the station's lines is running. **View all →** opens the full station page.
...
- **Sheet sizing & gestures:** The mid snap is **fitted to its own content**, not a fixed fraction — it measures the actions + departures block and rests exactly at that block's bottom edge, so an interchange's four cards all fit and the schedule below never peeks through the fold. It's floored so the sheet can never take more than ~88% of the screen. A tap on the header **climbs** the ladder (collapsed → mid → full, full → mid); a tap on the **map surface collapses** the sheet to its peek. Leaflet suppresses the map tap after a pan, and station markers don't trigger it — tapping a marker selects that station and raises the sheet to mid.
...
- **Station Detail Pages:** `/stations/:id` shows all live simulated departures per line. It is a real route (the SEO/deep-link surface) and is reached from the sheet's **View all →**; the sheet's own "Station Details" button expands in place instead (§4.1).
- **Shared body:** `StationDetailBody` renders the schedule for both the standalone page and the home sheet, so the two can't drift. Two flags adapt it: `showHero` (the sheet already names the station in its header) and `showActions` (the sheet supplies its own action pair, so the page's **From here / To here** buttons are suppressed there). Phase and stop-of-N remain on the standalone page's hero; they were dropped from the sheet's chip row.
