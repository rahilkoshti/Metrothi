...
async function overpass(query, label) {
  ...
  const res = await fetch(OVERPASS_URL, {
    ...
    headers: {
      ...
      // Overpass's Apache content-negotiation 406s requests with no Accept
      // header (Node's fetch sends none by default).
      'Accept': '*/*',
      'User-Agent': 'Metrothi-dev-script/1.0 (one-off track-geometry fetch; contact: koshtirahil@gmail.com)',
    },
