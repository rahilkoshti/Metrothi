 ...
 */
function computeStopTimeline(result: any): number[] {
  const stops: any[] = result.stops ?? [];
  ...
  switch (currentState) {
    ...
    case 'WALKING_TO_STATION': {
      ...
      if (activeCoords && source.lat != null) {
        const distToSource = haversineKm(activeCoords, source);
    ...
    case 'WAITING_FOR_TRAIN': {
      ...
      if (activeCoords && source.lat != null && stops[1].lat != null) {
        const distToSource = haversineKm(activeCoords, source);
        const distToNext = haversineKm(activeCoords, stops[1]);
        const srcToNext = haversineKm(source, stops[1]);
    ...
    case 'APPROACHING_DESTINATION': {
      ...
      if (activeCoords) {
        ...
        if (candidateStop && candidateStop.lat != null) {
          const dist = haversineKm(activeCoords, candidateStop);
      ...
      if (currentState === 'ON_TRAIN' && activeCoords) {
        ...
        if (nextStop && nextStop.lat != null) {
          const distToNext = haversineKm(activeCoords, nextStop);
    ...
    case 'TRANSFERRING': {
      ...
      if (activeCoords) {
        ...
        if (interchange.lat != null && nextStop && nextStop.lat != null) {
          const distToInterchange = haversineKm(activeCoords, interchange);
          const distToNext = haversineKm(activeCoords, nextStop);
          const interToNext = haversineKm(interchange, nextStop);
...
export function useJourneySession(result: any | null) {
  ...
  const resultIdRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    const sourceDistKm =
      result.source?.distanceKm != null ? (result.source.distanceKm as number) : null;
